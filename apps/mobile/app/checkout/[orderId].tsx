...
import { escrowService, CreateEscrowResponse } from '../../lib/api/services/escrow';
...
export default function CheckoutScreen() {
    ...
    const handlePayWithEscrow = async () => {
        ...
        try {
            ...
            // In a real app, you would use Stripe SDK to confirm payment here
    ...
    if (!order) {
        return (
            <Center flex={1} bg={COLORS.luxuryBlack}>
                ...
                <Text color={COLORS.textSecondary} mt="$4">Order not found</Text>
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.luxuryBlack }} edges={['top']}>
            ...
            {/* Header */}
            <Box px="$6" pt="$4" pb="$4" borderBottomWidth={1} borderBottomColor={COLORS.darkBorder}>
            ...
            <ScrollView flex={1} contentContainerStyle={{ paddingBottom: 120 }}>
                <VStack p="$6" space="lg">
                    {/* Error Message */}
                    {error && (
                        <Box bg={`${COLORS.errorRed}20`} p="$4" rounded="$lg" borderWidth={1} borderColor={COLORS.errorRed}>
                            <Text size="sm" color={COLORS.errorRed}>{error}</Text>
                        </Box>
                    )}

                    {/* Escrow Protection Banner */}
                    <Box bg={`${COLORS.successGreen}20`} p="$4" rounded="$xl" borderWidth={2} borderColor={COLORS.successGreen}>
                        <HStack alignItems="center" space="md">
                            <Box bg={COLORS.successGreen} p="$3" rounded="$full">
                                <Shield size={24} color={COLORS.textPrimary} />
                            </Box>
                            <VStack flex={1}>
                                <Text fontWeight="$bold" color={COLORS.textPrimary}>Buyer Protection</Text>
                                <Text size="xs" color={COLORS.textMuted}>
                    ...
                    {/* Order Summary */}
                    <Box bg={COLORS.luxuryBlackLight} rounded="$xl" borderWidth={2} borderColor={COLORS.darkBorder} overflow="hidden">
                        <Box bg={COLORS.primaryGold} px="$4" py="$3">
                            <Text color={COLORS.luxuryBlack} fontWeight="$bold">Order Summary</Text>
                        </Box>
                        <VStack p="$4" space="sm">
                            <HStack justifyContent="space-between">
                                <Text color={COLORS.textMuted}>Item Price</Text>
                                <Text fontWeight="$bold" color={COLORS.textPrimary}>${order.itemPrice}</Text>
                            </HStack>
                            <HStack justifyContent="space-between">
                                <Text color={COLORS.textMuted}>Shipping</Text>
                                <Text fontWeight="$bold" color={COLORS.textPrimary}>${order.shippingCost}</Text>
                            </HStack>
                            <HStack justifyContent="space-between">
                                <Text color={COLORS.textMuted}>Tax</Text>
                                <Text fontWeight="$bold" color={COLORS.textPrimary}>${order.tax}</Text>
                            </HStack>
                            <Box h={1} bg={COLORS.darkBorder} my="$2" />
                            <HStack justifyContent="space-between">
                                <Text fontWeight="$black" color={COLORS.textPrimary} size="lg">Total</Text>
                                <Text fontWeight="$black" color={COLORS.primaryGold} size="lg">${order.total}</Text>
                            </HStack>
                        </VStack>
                    </Box>
                    ...
                    {/* How Escrow Works */}
                    <Box>
                        <Text fontWeight="$bold" color={COLORS.textPrimary} mb="$3">How Escrow Protection Works</Text>
                        <VStack space="sm">
                            <HStack alignItems="flex-start" space="md">
                                <Box bg={COLORS.primaryGold} w={28} h={28} rounded="$full" alignItems="center" justifyContent="center">
                                    <Text color={COLORS.luxuryBlack} fontWeight="$bold" size="xs">1</Text>
                                </Box>
                                <VStack flex={1}>
                                    <Text fontWeight="$bold" color={COLORS.textPrimary} size="sm">Pay Securely</Text>
                                    <Text size="xs" color={COLORS.textMuted}>
                            ...
                            <HStack alignItems="flex-start" space="md">
                                <Box bg={COLORS.primaryGold} w={28} h={28} rounded="$full" alignItems="center" justifyContent="center">
                                    <Text color={COLORS.luxuryBlack} fontWeight="$bold" size="xs">2</Text>
                                </Box>
                                <VStack flex={1}>
                                    <Text fontWeight="$bold" color={COLORS.textPrimary} size="sm">Seller Ships Item</Text>
                                    <Text size="xs" color={COLORS.textMuted}>
                            ...
                            <HStack alignItems="flex-start" space="md">
                                <Box bg={COLORS.primaryGold} w={28} h={28} rounded="$full" alignItems="center" justifyContent="center">
                                    <Text color={COLORS.luxuryBlack} fontWeight="$bold" size="xs">3</Text>
                                </Box>
                                <VStack flex={1}>
                                    <Text fontWeight="$bold" color={COLORS.textPrimary} size="sm">Confirm Delivery</Text>
                                    <Text size="xs" color={COLORS.textMuted}>
                    ...
                    {/* Auto-release notice */}
                    <Box bg={`${COLORS.warningAmber}20`} p="$3" rounded="$lg" borderWidth={1} borderColor={COLORS.warningAmber}>
